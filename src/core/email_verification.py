...
class EmailVerificationHandler:
    ...
    def get_verification_code(self, max_retries=10, retry_interval=30):
        ...
        for attempt in range(max_retries):
            try:
                logging.info(f"帅哥,正在尝试获取验证码 (第 {attempt + 1}/{max_retries} 次)...")
                ...
                if attempt < max_retries - 1:  # 除了最后一次尝试，都等待
                    logging.warning(f"帅哥,还没收到验证码，{retry_interval} 秒后再试一次...")
                    time.sleep(retry_interval)

            except Exception as e:
                logging.error(f"帅哥,获取验证码失败了: {e}")  # 记录更一般的异常
                if attempt < max_retries - 1:
                    logging.error(f"帅哥,出了点问题，{retry_interval} 秒后再试一次...")
                    time.sleep(retry_interval)
                else:
                    raise Exception(f"帅哥,获取验证码失败且已达最大重试次数: {e}") from e

        raise Exception(f"帅哥,经过 {max_retries} 次尝试后还是没拿到验证码。")
            ...
            for part in email_message.walk():
                ...
                if content_type == "text/plain" and "attachment" not in content_disposition:
                    ...
                    try:
                        ...
                        logging.error(f"帅哥,解码邮件正文失败了: {e}")
        else:
    ...
    def _get_latest_mail_code(self):
        ...
        logging.info(f"帅哥,找到一封邮件,主题是: {mail_subject}")
